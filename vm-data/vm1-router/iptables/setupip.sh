...
# Make sure ssh is not blocked even though we don't touch the INPUT table 
...
# ENABLE NFQUEUE SCRIPT
iptables -t mangle -A POSTROUTING -p udp --sport 53 -j NFQUEUE --queue-num 1
iptables -t mangle -A POSTROUTING -p udp --sport 553 -j NFQUEUE --queue-num 1
